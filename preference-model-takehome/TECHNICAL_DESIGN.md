...
### 1.1 RL Training Context

This task is designed as an **environment** for reinforcement learning training of language models. In the RL framework:

**Environment Properties:**
- **State:** Initial buggy implementation (dropout with wrong scaling)
- **Action Space:** Code modifications (edit dropout.py)
- **Reward Function:** Binary signal based on statistical tests
  - R = +1 if variance preserved within 10% tolerance
  - R = 0 if variance error > 10%
- **Episode Termination:** After grading (deterministic)
- **Observation:** Task prompt + buggy code + test file

**Why This Works for RL:**
1. **Clear reward signal:** 43% error vs 0.1% error provides strong gradient
2. **Deterministic grading:** Same fix always gets same reward (reproducible)
3. **Sparse but informative:** Binary reward, but failure modes are diagnostic
4. **Skill transfer:** Learning to preserve variance generalizes to other statistical properties

**Integration with RL Pipeline:**
```python
# Simplified RL training loop
state = setup_task()  # Creates buggy dropout.py
action = policy(state)  # Model edits code
reward = grade_solution(action)  # Statistical validation
update_policy(reward)  # RL update
```
